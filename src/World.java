...
/**
 * Class World - Reads a .txt file with specific block ids
 * then renders them to the display
 * 
...
public class World {
    ...
    int xStart =  (int) Math.max(0, display.getCamera().getXCamera()/WorldBlocks.blockWidth);  //Either the first block is rendered or the shift on camera divided by the blockwidth blocks are rendered.
    int yStart = (int) Math.max(0, display.getCamera().getYCamera()/WorldBlocks.blockHeigth); 
    int xEnd = (int) Math.min(width, (display.getCamera().getXCamera() + display.getWidth())/ WorldBlocks.blockWidth +1 ); // you add the display width because u want the far right of the screen
    int yEnd = (int) Math.min(heigth, (display.getCamera().getYCamera() + display.getWidth())/ WorldBlocks.blockHeigth +1 );
    ...
    private void loadWorld(String path){
       ...
       for(int y = 0; y <heigth ; y++){ // read the rest
           for(int x = 0; x < width; x++){
               blocks[x][y] = readFile.parseInt(tokens[(x +y *width)+ 4]); // stackOverFlow, fixes the offset of the blocks.
           }
       }
